...
Clone the git repositories for AchieveCoin and Gitian.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/AchieveCoin/.git/
...
For example:
```bash
URL=https://github.com/akx20000a/AchieveCoin.git
COMMIT=d5522ed66a924a2d505fe45ed2487e741c6c0917
./bin/gbuild --commit AchieveCoin=${COMMIT} --url AchieveCoin=${URL} ../AchieveCoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit AchieveCoin=${COMMIT} --url AchieveCoin=${URL} ../AchieveCoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit AchieveCoin=${COMMIT} --url AchieveCoin=${URL} ../AchieveCoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
BTCPATH=/some/root/path/AchieveCoin
SIGPATH=/some/root/path/achievecoin-detached-sigs

./bin/gbuild --url AchieveCoin=${BTCPATH},signature=${SIGPATH} ../AchieveCoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/achievecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/achievecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/achievecoin-osx-build.assert
```
...
[AchieveCoin/gitian.sigs](https://github.com/AchieveCoin/gitian.sigs/) repository, or if that's not possible create a pull
request.
